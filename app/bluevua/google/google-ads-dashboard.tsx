...
export function GoogleAdsDashboard({ initialData }: { initialData: GoogleAdsDashboardData }) {
  ...
  return <div className="min-h-screen bg-background-secondary-default lg:flex">
    <aside className={cx("fixed inset-y-0 left-0 z-30 w-72 border-r border-separator-border bg-background-primary-default p-4 transition-transform lg:sticky lg:top-0 lg:h-screen lg:translate-x-0", mobileNav ? "translate-x-0" : "-translate-x-full")}><div className="flex h-full flex-col"><SidebarProjectLink reportName="Google Dashboard" /><nav className="mt-6 flex flex-col gap-1" aria-label="Dashboard sections">{nav.map((item,i) => { const Icon=icons[i]; return <button key={item} className={cx("flex items-center gap-3 rounded-xl px-3 py-2.5 text-left text-body-medium", i===0 ? "bg-button-ghost-background text-button-ghost-foreground shadow-nav-selected" : "text-text-secondary hover:bg-background-primary-hover hover:text-text-primary")}><Icon className="size-5" />{item}</button>})}</nav><div className="mt-auto rounded-2xl bg-background-secondary-default p-4"><p className="text-caption-1-semibold text-text-tertiary">LAST UPDATED</p><p className="mt-2 text-body-medium text-text-primary">{formatDate(data.latestDate)} · 5:00 AM ET</p><p className="mt-1 text-caption-2-regular text-text-secondary">Refreshes daily</p></div></div></aside>
