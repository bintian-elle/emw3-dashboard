...
function PlatformHeader({platform}:{platform:(typeof platforms)[number]}){const Icon=platform.icon;return <header className="flex items-center gap-4 px-2 py-1"><span className={cx("flex size-11 shrink-0 items-center justify-center rounded-xl",platform.tone)}><Icon className="size-6" aria-hidden/></span><div className="min-w-0 flex-1"><h2 className="text-title-2-semibold text-text-primary">{platform.name}</h2><p className="mt-1 text-body-regular text-text-secondary">{platform.description}</p></div></header>;}
...
export default async function TestingOverviewPage({searchParams}:{searchParams:Promise<SearchParams>}){
  ...
  return <><TestingPageHeader title="Testing Overview" description="Combined performance for the connected creative testing channels."/><BusinessPerformance google={google} demandGen={demandGen} meta={meta} reddit={reddit} edm={edm}/><section className="mt-10 rounded-3xl border border-border-button-default bg-background-secondary-default p-4" aria-label="Google performance"><PlatformHeader platform={platforms[0]}/><div className="mt-5"><GoogleTestSection test={google} compact/></div><DemandGenSection data={demandGen} query={query}/></section><section className="mt-6 rounded-3xl border border-border-button-default bg-background-secondary-default p-4" aria-label="Meta performance"><PlatformHeader platform={platforms[1]}/><div className="mt-5"><MetaCampaignSection data={meta} query={query}/></div></section><section className="mt-6 rounded-3xl border border-border-button-default bg-background-secondary-default p-4" aria-label="Reddit performance"><PlatformHeader platform={platforms[2]}/><RedditCampaignSection data={reddit} query={query}/></section><section className="mt-6 rounded-3xl border border-border-button-default bg-background-secondary-default p-4" aria-label="EDM performance"><PlatformHeader platform={platforms[3]}/><EdmCampaignSection data={edm}/></section><footer className="py-8 text-center text-caption-2-regular text-text-tertiary">Connected channels: Google · Meta · Reddit · EDM · Source: Supabase, Meta Ads, Reddit Ads and Klaviyo</footer></>;
}
