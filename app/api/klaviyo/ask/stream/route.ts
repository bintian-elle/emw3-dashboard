...
import { askCodex, CodexBridgeError, type BridgeProgress } from "@/lib/codex-bridge";

type ChatTurn={role:"user"|"assistant";content:string};
type StreamRequest=DashboardRequest&{question?:string;history?:ChatTurn[]};
  ...
  start(controller){
   ...
   void(async()=>{
    try{
     ...
     const result=await askCodex(input,context,contextualQuestion,request.signal,onProgress);
